...
class KubernetesControllerService:
    ...
    def check_if_exists(self, context, agentId):
    ...
    def get_running_agents(self, context: str = None):
        ...
        while(tries < 3):
            try:
                ...
                if context == None:
                    #pod_list = self.api_instance.list_namespaced_pod(self.namespace, label_selector="opentwins.agents/kind=ot-agent, opentwins.agents/type=deployment",watch=False)
                    deployment_list = self.k8s_apps_v1.list_namespaced_deployment(self.namespace, label_selector="opentwins.agents/kind=ot-agent")   
                    cronjob_list = self.batch_api.list_namespaced_cron_job(self.namespace, label_selector="opentwins.agents/kind=ot-agent")

                else:
                    #pod_list = self.api_instance.list_namespaced_pod(self.namespace, label_selector="opentwins.agents/kind=ot-agent, opentwins.agents/type=deployment, opentwins.agents/context={}".format(context),watch=False)
                    deployment_list = self.k8s_apps_v1.list_namespaced_deployment(self.namespace, label_selector="opentwins.agents/kind=ot-agent, opentwins.agents/context={}".format(context))                       
                    cronjob_list = self.batch_api.list_namespaced_cron_job(self.namespace, label_selector="opentwins.agents/kind=ot-agent, opentwins.agents/context={}".format(context))
                ...
                for deployment in deployment_list.items:
                    ...
                    #json.dumps(pod.metadata.creation_timestamp, default=str),
